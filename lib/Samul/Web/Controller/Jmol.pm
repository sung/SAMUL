...
# /jmol/pdb/xx
# (a frame called from /pdb/1a25/jmol, /pdb/1a25/jmol?hl=2:A,11:B&label=ACT_SITE,BINDING)
sub pdb: Local{
    my ($self, $c, $id) = @_;
	unless(defined $id){
	    $c->stash->{template} = 'Search/form.tt2';
...
sub _load_scop_sql: Private{
	...
	# $rs=101321 (The number of entries in SAMUL.Scopmap)
	# my $rs=$c->model('SAMUL::Scopmap');	# ResultSet?
	...
	# 
	my $rs=$c->model('SAMUL');	# ResultSource ?
	my $schema=$rs->schema;
...
sub _load_scop: Private{
    ...
    # where pdb=?
    # group by sid 
    $c->stash->{domains} = [$c->model('SAMUL::Scopmap')->search(
        {
            'pdb'=>$c->stash->{id},
        },
        {
...
sub _load_interface: Private{
	...
	return $c->res->output('No psimap_dir') unless -d $psimap_dir;
...
sub _load_ligands: Private{
	my ($self, $c) = @_;
	my $ResMap=$c->config->{d_resmap};

=credo
select f.des, group_concat(concat(concat_ws('', m.pdb_res_num, m.ins_code), ':', m.pdb_chain_id) separator ', ') as des
from GLORIA.FuncRes f
join GLORIA.ResMap m on f.res_id=m.res_id
where m.pdb='1a25' and f.type='CREDO'
group by f.des
=cut

	$c->stash->{ligands} = [$c->model('GLORIA::Funcres')->search(
		{
			'me.type'=>'CREDO',
 			"$ResMap.pdb"=>$c->stash->{id},
		},
		{
			select=>[{distinct=>'me.des'}], 
      as=>[qw/des/],
			join=>[$ResMap],
		}
	)];
